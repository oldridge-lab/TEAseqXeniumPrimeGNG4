...
tc654b_path <- "/filepath/xenium_data_processing/xenium_xoa_output_folder/tc654b/"
...
tc654b.data <- ReadXenium(tc654b_path, outs = c('matrix','microns'),
                          type = c('segmentations', 'centroids'),
                          mols.qv.threshold = 20)

# Finish Seurat object creation using steps normally in LoadXenium function
tc654b.segmentations <- list(centroids = CreateCentroids(tc654b.data$centroids), 
                             segmentation = CreateSegmentation(tc654b.data$segmentations))
tc654b.coords <- CreateFOV(coords = tc654b.segmentations, 
                           type = c("segmentation","centroids"),
                           molecules = tc654b.data$microns, assay = "RNA")
tc654b.obj <- CreateSeuratObject(counts = tc654b.data$matrix[["Gene Expression"]], assay = "RNA")
tc654b.obj[["fov"]] <- tc654b.coords

# Add quality control data from XOA as separate assays to object
tc654b.obj[["BlankCodeword"]] <- CreateAssayObject(counts = tc654b.data$matrix[["Unassigned Codeword"]])
tc654b.obj[["ControlCodeword"]] <- CreateAssayObject(counts = tc654b.data$matrix[["Negative Control Codeword"]])
tc654b.obj[["ControlProbe"]] <- CreateAssayObject(counts = tc654b.data$matrix[["Negative Control Probe"]])
tc654b.obj[["GenomicControl"]] <- CreateAssayObject(counts = tc654b.data$matrix[["Genomic Control"]])
tc654b.obj[["DeprecatedCodeword"]] <- CreateAssayObject(counts = tc654b.data$matrix[["Deprecated Codeword"]])

# Add donor metadata
tc654b.obj@project.name <- 'TC654B'
tc654b.obj$donor <- 'TC654B'
tc654b.obj$age <- 8
tc654b.obj$asab <- 'Female'
tc654b.obj$indication <- 'SDB'
tc654b.obj$slide <- 'Slide1'
Idents(tc654b.obj) <- 'donor'

# Save preprocessed object 
saveRDS(tc654b.obj, file = '/filepath/xenium_data_processing/step1_preprocess/tc654b.preproc.seurat.obj.rds')
