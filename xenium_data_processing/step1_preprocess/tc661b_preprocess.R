...
tc661b_path <- "/filepath/xenium_data_processing/xenium_xoa_output_folder/tc661b/"
...
tc661b.data <- ReadXenium(tc661b_path, outs = c('matrix','microns'),
                          type = c('segmentations', 'centroids'),
                          mols.qv.threshold = 20)

# Finish Seurat object creation using steps normally in LoadXenium function
tc661b.segmentations <- list(centroids = CreateCentroids(tc661b.data$centroids), 
                             segmentation = CreateSegmentation(tc661b.data$segmentations))
tc661b.coords <- CreateFOV(coords = tc661b.segmentations, 
                           type = c("segmentation","centroids"),
                           molecules = tc661b.data$microns, assay = "RNA")
tc661b.obj <- CreateSeuratObject(counts = tc661b.data$matrix[["Gene Expression"]], assay = "RNA")
tc661b.obj[["fov"]] <- tc661b.coords

# Add quality control data from XOA as separate assays to object
tc661b.obj[["BlankCodeword"]] <- CreateAssayObject(counts = tc661b.data$matrix[["Unassigned Codeword"]])
tc661b.obj[["ControlCodeword"]] <- CreateAssayObject(counts = tc661b.data$matrix[["Negative Control Codeword"]])
tc661b.obj[["ControlProbe"]] <- CreateAssayObject(counts = tc661b.data$matrix[["Negative Control Probe"]])
tc661b.obj[["GenomicControl"]] <- CreateAssayObject(counts = tc661b.data$matrix[["Genomic Control"]])
tc661b.obj[["DeprecatedCodeword"]] <- CreateAssayObject(counts = tc661b.data$matrix[["Deprecated Codeword"]])

# Add donor metadata
tc661b.obj@project.name <- 'TC661B'
tc661b.obj$donor <- 'TC661B'
tc661b.obj$age <- 8
tc661b.obj$asab <- 'Female'
tc661b.obj$indication <- 'SDB'
tc661b.obj$slide <- 'Slide2'
Idents(tc661b.obj) <- 'donor'

# Save preprocessed object 
saveRDS(tc661b.obj, file = '/filepath/xenium_data_processing/step1_preprocess/tc661b.preproc.seurat.obj.rds')
