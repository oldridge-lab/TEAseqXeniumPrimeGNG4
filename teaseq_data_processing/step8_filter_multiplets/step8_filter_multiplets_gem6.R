...
gem6.hto <- readRDS('/filepath/step7_hto_demultiplex/gem6.hto.rds')
...
DefaultAssay(gem6.hto) <- 'RNA'
gem6.sce <- as.SingleCellExperiment(gem6.hto)

# Run scDblFinder on RNA data
scDbl.rna_gem6 <- scDblFinder(gem6.sce, dbr = dbr.est) 

# Append scDblFinder outputs from RNA modality to original Seurat object
gem6.hto$scDblF.RNA.score <- scDbl.rna_gem6$scDblFinder.score
gem6.hto$scDblF.RNA.class <- scDbl.rna_gem6$scDblFinder.class
gem6.hto$scDblF.RNA.class <- ifelse(gem6.hto$scDblF.RNA.class == "singlet", "singlet", "multiplet")

# 4) Run scDblFinder using ATAC modality ----

DefaultAssay(gem6.hto) <- 'ATAC'
gem6.sce <- as.SingleCellExperiment(gem6.hto)
scDbl.atac_gem6 <- scDblFinder(gem6.sce, dbr = dbr.est, aggregateFeatures = TRUE, nfeatures = 25, processing = 'normFeatures') 

# Append scDblFinder outputs from ATAC modality to original Seurat object
gem6.hto$scDblF.ATAC.score <- scDbl.atac_gem6$scDblFinder.score
gem6.hto$scDblF.ATAC.class <- scDbl.atac_gem6$scDblFinder.class
gem6.hto$scDblF.ATAC.class <- ifelse(gem6.hto$scDblF.ATAC.class == "singlet", "singlet", "multiplet")

# 5) Determine multiplet status from % HTO UMI metrics ----

# Create dataframe containing % UMI from maximum, 2nd, and cumulative 2nd-16th HTO per cell
pct.hto.df <- data.frame(gem6.hto$pct.hto.max, gem6.hto$pct.hto.2nd, gem6.hto$pct.hto.bg)
...
gem6.hto$hto.class <- pct.hto.df$hto.class
...
pct.hto.df %>% ggplot(aes(x = hto.class, fill = hto.class)) + 
  ...
  labs(title = '% HTO UMI Assignment (GEM6)',
       x = 'Assignment',
       y = 'Number of cells') +
  theme_minimal()
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.2nd`, color = hto.class)) +
  ...
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd HTO (GEM6)") +
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.bg`, color = hto.class)) +
  ...
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd-16th HTO (GEM6)") +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
  labs(color = "Droplet")
...
gem6.hto$hto.class <- pct.hto.df$hto.class
Idents(gem6.hto) <- 'hto.class'
gem6.hto@active.ident <- factor(gem6.hto@active.ident, levels = c('singlet','multiplet','negative'))
VlnPlot(gem6.hto, features = 'nCount_HTO', pt.size = 0.1, log = TRUE, cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
VlnPlot(gem6.hto, features = 'nCount_ADT', pt.size = 0.1, log = TRUE, cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
VlnPlot(gem6.hto, features = 'nCount_RNA', pt.size = 0.1, log = TRUE,  cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
VlnPlot(gem6.hto, features = 'nCount_ATAC', pt.size = 0.1, log = TRUE, cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()

# 6) Analyze scDblFinder RNA performance ----

# Add scDblFinder RNA classifications to % HTO UMI dataframe
pct.hto.df$scDblF.RNA.class <- gem6.hto$scDblF.RNA.class
...
pct.hto.df %>% ggplot(aes(x = scDblF.RNA.class, fill = scDblF.RNA.class)) + 
  ...
  scale_fill_manual(values = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'scDblF.RNA.class Assignment (GEM6)',
       x = 'Assignment',
       y = 'Number of cells') +
  theme_minimal()
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.2nd`, color = scDblF.RNA.class)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd HTO - colored by scDblF.RNA.class classification (GEM6)") +
  geom_hline(yintercept = 15) +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.bg`, color = scDblF.RNA.class)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd-16th HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd-16th HTO - colored by scDblF.RNA.class classification (GEM6)") +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
  labs(color = "scDblF.RNA.class")
...
Idents(gem6.hto) <- 'scDblF.RNA.class'
gem6.hto@active.ident <- factor(gem6.hto@active.ident, levels = c('singlet','multiplet','negative'))
VlnPlot(gem6.hto, features = 'nCount_HTO', pt.size = 0.1, log = TRUE, cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
VlnPlot(gem6.hto, features = 'nCount_ADT', pt.size = 0.1, log = TRUE, cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
VlnPlot(gem6.hto, features = 'nCount_RNA', pt.size = 0.1, log = TRUE,  cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
...
# Add scDblFinder RNA classifications to % HTO UMI dataframe
pct.hto.df$scDblF.ATAC.class <- gem6.hto$scDblF.ATAC.class
...
pct.hto.df %>% ggplot(aes(x = scDblF.ATAC.class, fill = scDblF.ATAC.class)) + 
  ...
  scale_fill_manual(values = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'scDblF.ATAC.class Assignment (GEM6)',
       x = 'Assignment',
       y = 'Number of cells') +
  theme_minimal()
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.2nd`, color = scDblF.ATAC.class)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd HTO - colored by scDblF.ATAC.class classification (GEM6)") +
  geom_hline(yintercept = 15) +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.bg`, color = scDblF.ATAC.class)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd-16th HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd-16th HTO - colored by scDblF.ATAC.class classification (GEM6)") +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
  labs(color = "scDblF.ATAC.class")
...
Idents(gem6.hto) <- 'scDblF.ATAC.class'
gem6.hto@active.ident <- factor(gem6.hto@active.ident, levels = c('singlet','multiplet','negative'))
VlnPlot(gem6.hto, features = 'nCount_HTO', pt.size = 0.1, log = TRUE, cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
VlnPlot(gem6.hto, features = 'nCount_ADT', pt.size = 0.1, log = TRUE, cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
VlnPlot(gem6.hto, features = 'nCount_RNA', pt.size = 0.1, log = TRUE,  cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
...
gem6.hto$snp.class <- gem6.hto$status
gem6.hto@meta.data <- gem6.hto@meta.data %>%
  mutate(snp.class = recode(snp.class, 
                                 'unassigned' = 'negative', 
                                 'doublet' = 'multiplet'))
gem6.hto$snp.profile <- gem6.hto$assignment
gem6.hto$snp.singlet.logp <- gem6.hto$log_prob_singleton
gem6.hto$snp.multip.logp <- gem6.hto$log_prob_doublet
gem6.hto$snp.profile0.logp <- gem6.hto$cluster0
gem6.hto$snp.profile1.logp <- gem6.hto$cluster1
gem6.hto$snp.profile2.logp <- gem6.hto$cluster2
gem6.hto$snp.profile3.logp <- gem6.hto$cluster3
gem6.hto$snp.profile4.logp <- gem6.hto$cluster4
gem6.hto$snp.profile5.logp <- gem6.hto$cluster5
gem6.hto$snp.profile6.logp <- gem6.hto$cluster6
gem6.hto$snp.profile7.logp <- gem6.hto$cluster7

# Clean up excess Souporcell metadata
gem6.hto$status <- NULL
gem6.hto$assignment <- NULL
gem6.hto$log_prob_singleton <- NULL
gem6.hto$log_prob_doublet <- NULL
gem6.hto$cluster0 <- NULL
gem6.hto$cluster1 <- NULL
gem6.hto$cluster2 <- NULL
gem6.hto$cluster3 <- NULL
gem6.hto$cluster4 <- NULL
gem6.hto$cluster5 <- NULL
gem6.hto$cluster6 <- NULL
gem6.hto$cluster7 <- NULL

# Add Souporcell classifications to % HTO UMI dataframe
pct.hto.df$snp.class <- gem6.hto$snp.class
...
pct.hto.df %>% ggplot(aes(x = snp.class, fill = snp.class)) + 
  ...
  scale_fill_manual(values = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Souporcell Assignment (GEM6)',
       x = 'Assignment',
       y = 'Number of cells') +
  theme_minimal()
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.2nd`, color = snp.class)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd HTO - colored by Souporcell classification (GEM6)") +
  geom_hline(yintercept = 15) +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.bg`, color = snp.class)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd-16th HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd-16th HTO - colored by Souporcell classification (GEM6)") +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
  labs(color = "snp.class")
...
pct.hto.df %>% ggplot(aes(x = any.multip, fill = any.multip)) + 
  ...
  scale_fill_manual(values = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Combined multiplet assignment (GEM6)',
       x = 'Assignment',
       y = 'Number of cells') +
  theme_minimal()
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.2nd`, color = any.multip)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd HTO - colored by combined multiplet assignment (GEM6)") +
  geom_hline(yintercept = 15) +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.bg`, color = any.multip)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd-16th HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd-16th HTO - colored by combined multiplet assignment (GEM6)") +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
  labs(color = "any.multip")
...
gem6.hto$joint.multip <- pct.hto.df$joint.multip
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.2nd`, color = joint.multip)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd HTO - colored by consensus multiplet assignment (GEM6)") +
  geom_hline(yintercept = 15) +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
...
gem6.multip <- gem6.hto
saveRDS(gem6.multip, '/filepath/step8_filter_multiplets/gem6.multip.rds')

# Import multiplet object if starting from here
gem6.hto <- readRDS('/filepath/step8_filter_multiplets/gem6.multip.rds')

# Filter out multiplets and unclassified cells
gem6.singlet <- subset(gem6.hto, subset = 
                         hto.class == 'singlet' &
                         snp.class == 'singlet' &
                         scDblF.RNA.class == 'singlet' &
...
quantile(gem6.singlet$nCount_HTO, 0.999)
quantile(gem6.singlet$nCount_ADT, 0.999)
quantile(gem6.singlet$nCount_RNA, 0.999)
quantile(gem6.singlet$nFeature_RNA, 0.999)
quantile(gem6.singlet$nCount_ATAC, 0.999)
quantile(gem6.singlet$nFeature_ATAC, 0.999)

# Visualize outlier cutoff for UMI counts across modalities
Idents(gem6.singlet) <- 'hto.sort'
VlnPlot(gem6.singlet, features = 'nCount_HTO', pt.size = 0.1, log = TRUE) + NoLegend() + geom_hline(yintercept = 4000)
VlnPlot(gem6.singlet, features = 'nCount_ADT', pt.size = 0.1, log = TRUE) + NoLegend() + geom_hline(yintercept = 4500)
VlnPlot(gem6.singlet, features = 'nCount_RNA', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 20000)
VlnPlot(gem6.singlet, features = 'nFeature_RNA', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 5000)
VlnPlot(gem6.singlet, features = 'nCount_ATAC', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 50000)
VlnPlot(gem6.singlet, features = 'nFeature_ATAC', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 15000)

# Filter outlier UMI droplets from object
gem6.singlet.umi <- subset(gem6.singlet, subset = 
                             nCount_HTO < 4000 &
                             nCount_ADT < 4500 &
                             nCount_RNA < 20000 &
                             nFeature_RNA < 5000 &
...
(ncol(gem6.hto)) # 5643 input
(ncol(gem6.singlet)) # 4458 singlets
(ncol(gem6.singlet) - ncol(gem6.singlet.umi)) # 11 removed
(ncol(gem6.singlet.umi)) # 4447 remaining

# Visualize new UMI distribution across modalities
VlnPlot(gem6.singlet.umi, features = 'nCount_HTO', pt.size = 0.1, log = TRUE) + NoLegend() + geom_hline(yintercept = 4000) + ylim(0,4500)
VlnPlot(gem6.singlet.umi, features = 'nCount_ADT', pt.size = 0.1, log = TRUE) + NoLegend() + geom_hline(yintercept = 4500) + ylim(0,5000)
VlnPlot(gem6.singlet.umi, features = 'nCount_RNA', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 20000) + ylim(0,20500)
VlnPlot(gem6.singlet.umi, features = 'nFeature_RNA', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 5000) + ylim(0,5500)
VlnPlot(gem6.singlet.umi, features = 'nCount_ATAC', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 50000) + ylim(0,50500)
VlnPlot(gem6.singlet.umi, features = 'nFeature_ATAC', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 15000) + ylim(0,15500)

# 12) Save singlet Seurat object ----
saveRDS(gem6.singlet.umi, '/filepath/step8_filter_multiplets/gem6.singlet.umi.rds')
