...
gem2.hto <- readRDS('/filepath/step7_hto_demultiplex/gem2.hto.rds')
...
DefaultAssay(gem2.hto) <- 'RNA'
gem2.sce <- as.SingleCellExperiment(gem2.hto)

# Run scDblFinder on RNA data
scDbl.rna_gem2 <- scDblFinder(gem2.sce, dbr = dbr.est) 

# Append scDblFinder outputs from RNA modality to original Seurat object
gem2.hto$scDblF.RNA.score <- scDbl.rna_gem2$scDblFinder.score
gem2.hto$scDblF.RNA.class <- scDbl.rna_gem2$scDblFinder.class
gem2.hto$scDblF.RNA.class <- ifelse(gem2.hto$scDblF.RNA.class == "singlet", "singlet", "multiplet")

# 4) Run scDblFinder using ATAC modality ----

DefaultAssay(gem2.hto) <- 'ATAC'
gem2.sce <- as.SingleCellExperiment(gem2.hto)
scDbl.atac_gem2 <- scDblFinder(gem2.sce, dbr = dbr.est, aggregateFeatures = TRUE, nfeatures = 25, processing = 'normFeatures') 

# Append scDblFinder outputs from ATAC modality to original Seurat object
gem2.hto$scDblF.ATAC.score <- scDbl.atac_gem2$scDblFinder.score
gem2.hto$scDblF.ATAC.class <- scDbl.atac_gem2$scDblFinder.class
gem2.hto$scDblF.ATAC.class <- ifelse(gem2.hto$scDblF.ATAC.class == "singlet", "singlet", "multiplet")

# 5) Determine multiplet status from % HTO UMI metrics ----

# Create dataframe containing % UMI from maximum, 2nd, and cumulative 2nd-16th HTO per cell
pct.hto.df <- data.frame(gem2.hto$pct.hto.max, gem2.hto$pct.hto.2nd, gem2.hto$pct.hto.bg)
...
gem2.hto$hto.class <- pct.hto.df$hto.class
...
pct.hto.df %>% ggplot(aes(x = hto.class, fill = hto.class)) + 
  ...
  labs(title = '% HTO UMI Assignment (GEM2)',
       x = 'Assignment',
       y = 'Number of cells') +
  theme_minimal()
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.2nd`, color = hto.class)) +
  ...
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd HTO (GEM2)") +
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.bg`, color = hto.class)) +
  ...
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd-16th HTO (GEM2)") +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
  labs(color = "Droplet")
...
gem2.hto$hto.class <- pct.hto.df$hto.class
Idents(gem2.hto) <- 'hto.class'
gem2.hto@active.ident <- factor(gem2.hto@active.ident, levels = c('singlet','multiplet','negative'))
VlnPlot(gem2.hto, features = 'nCount_HTO', pt.size = 0.1, log = TRUE, cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
VlnPlot(gem2.hto, features = 'nCount_ADT', pt.size = 0.1, log = TRUE, cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
VlnPlot(gem2.hto, features = 'nCount_RNA', pt.size = 0.1, log = TRUE,  cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
VlnPlot(gem2.hto, features = 'nCount_ATAC', pt.size = 0.1, log = TRUE, cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()

# 6) Analyze scDblFinder RNA performance ----

# Add scDblFinder RNA classifications to % HTO UMI dataframe
pct.hto.df$scDblF.RNA.class <- gem2.hto$scDblF.RNA.class
...
pct.hto.df %>% ggplot(aes(x = scDblF.RNA.class, fill = scDblF.RNA.class)) + 
  ...
  scale_fill_manual(values = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'scDblF.RNA.class Assignment (GEM2)',
       x = 'Assignment',
       y = 'Number of cells') +
  theme_minimal()
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.2nd`, color = scDblF.RNA.class)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd HTO - colored by scDblF.RNA.class classification (GEM2)") +
  geom_hline(yintercept = 15) +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.bg`, color = scDblF.RNA.class)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd-16th HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd-16th HTO - colored by scDblF.RNA.class classification (GEM2)") +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
  labs(color = "scDblF.RNA.class")
...
Idents(gem2.hto) <- 'scDblF.RNA.class'
gem2.hto@active.ident <- factor(gem2.hto@active.ident, levels = c('singlet','multiplet','negative'))
VlnPlot(gem2.hto, features = 'nCount_HTO', pt.size = 0.1, log = TRUE, cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
VlnPlot(gem2.hto, features = 'nCount_ADT', pt.size = 0.1, log = TRUE, cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
VlnPlot(gem2.hto, features = 'nCount_RNA', pt.size = 0.1, log = TRUE,  cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
...
# Add scDblFinder RNA classifications to % HTO UMI dataframe
pct.hto.df$scDblF.ATAC.class <- gem2.hto$scDblF.ATAC.class
...
pct.hto.df %>% ggplot(aes(x = scDblF.ATAC.class, fill = scDblF.ATAC.class)) + 
  ...
  scale_fill_manual(values = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'scDblF.ATAC.class Assignment (GEM2)',
       x = 'Assignment',
       y = 'Number of cells') +
  theme_minimal()
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.2nd`, color = scDblF.ATAC.class)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd HTO - colored by scDblF.ATAC.class classification (GEM2)") +
  geom_hline(yintercept = 15) +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.bg`, color = scDblF.ATAC.class)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd-16th HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd-16th HTO - colored by scDblF.ATAC.class classification (GEM2)") +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
  labs(color = "scDblF.ATAC.class")
...
Idents(gem2.hto) <- 'scDblF.ATAC.class'
gem2.hto@active.ident <- factor(gem2.hto@active.ident, levels = c('singlet','multiplet','negative'))
VlnPlot(gem2.hto, features = 'nCount_HTO', pt.size = 0.1, log = TRUE, cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
VlnPlot(gem2.hto, features = 'nCount_ADT', pt.size = 0.1, log = TRUE, cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
VlnPlot(gem2.hto, features = 'nCount_RNA', pt.size = 0.1, log = TRUE,  cols = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + NoLegend()
...
gem2.hto$snp.class <- gem2.hto$status
gem2.hto@meta.data <- gem2.hto@meta.data %>%
  mutate(snp.class = recode(snp.class, 
                                 'unassigned' = 'negative', 
                                 'doublet' = 'multiplet'))
gem2.hto$snp.profile <- gem2.hto$assignment
gem2.hto$snp.singlet.logp <- gem2.hto$log_prob_singleton
gem2.hto$snp.multip.logp <- gem2.hto$log_prob_doublet
gem2.hto$snp.profile0.logp <- gem2.hto$cluster0
gem2.hto$snp.profile1.logp <- gem2.hto$cluster1
gem2.hto$snp.profile2.logp <- gem2.hto$cluster2
gem2.hto$snp.profile3.logp <- gem2.hto$cluster3
gem2.hto$snp.profile4.logp <- gem2.hto$cluster4
gem2.hto$snp.profile5.logp <- gem2.hto$cluster5
gem2.hto$snp.profile6.logp <- gem2.hto$cluster6
gem2.hto$snp.profile7.logp <- gem2.hto$cluster7

# Clean up excess Souporcell metadata
gem2.hto$status <- NULL
gem2.hto$assignment <- NULL
gem2.hto$log_prob_singleton <- NULL
gem2.hto$log_prob_doublet <- NULL
gem2.hto$cluster0 <- NULL
gem2.hto$cluster1 <- NULL
gem2.hto$cluster2 <- NULL
gem2.hto$cluster3 <- NULL
gem2.hto$cluster4 <- NULL
gem2.hto$cluster5 <- NULL
gem2.hto$cluster6 <- NULL
gem2.hto$cluster7 <- NULL

# Add Souporcell classifications to % HTO UMI dataframe
pct.hto.df$snp.class <- gem2.hto$snp.class
...
pct.hto.df %>% ggplot(aes(x = snp.class, fill = snp.class)) + 
  ...
  scale_fill_manual(values = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Souporcell Assignment (GEM2)',
       x = 'Assignment',
       y = 'Number of cells') +
  theme_minimal()
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.2nd`, color = snp.class)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd HTO - colored by Souporcell classification (GEM2)") +
  geom_hline(yintercept = 15) +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.bg`, color = snp.class)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd-16th HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd-16th HTO - colored by Souporcell classification (GEM2)") +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
  labs(color = "snp.class")
...
pct.hto.df %>% ggplot(aes(x = any.multip, fill = any.multip)) + 
  ...
  scale_fill_manual(values = c('singlet' = 'skyblue2', 'multiplet' = 'red3', 'negative' = 'grey')) + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Combined multiplet assignment (GEM2)',
       x = 'Assignment',
       y = 'Number of cells') +
  theme_minimal()
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.2nd`, color = any.multip)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd HTO - colored by combined multiplet assignment (GEM2)") +
  geom_hline(yintercept = 15) +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.bg`, color = any.multip)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd-16th HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd-16th HTO - colored by combined multiplet assignment (GEM2)") +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
  labs(color = "any.multip")
...
gem2.hto$joint.multip <- pct.hto.df$joint.multip
...
ggplot(pct.hto.df, aes(x = `pct.hto.max`, y = `pct.hto.2nd`, color = joint.multip)) +
  ...
  labs(x = "% UMI from Max HTO", y = "% UMI from 2nd HTO") +
  theme_minimal() +
  ggtitle("% UMI from Max vs 2nd HTO - colored by consensus multiplet assignment (GEM2)") +
  geom_hline(yintercept = 15) +
  geom_vline(xintercept = 50) +
  scale_color_manual(values = c("multiplet" = "red3", "singlet" = "skyblue2", "negative" = "grey")) +
  coord_fixed() +
...
# 10) Filter out multiplets and unclassified cells from Seurat object ----

# Save Seurat object with all multiplet metrics appended before filtering
gem2.multip <- gem2.hto
saveRDS(gem2.multip, '/filepath/step8_filter_multiplets/gem2.multip.rds')

# Import multiplet object if starting from here
gem2.hto <- readRDS('/filepath/step8_filter_multiplets/gem2.multip.rds')

# Filter out multiplets and unclassified cells
gem2.singlet <- subset(gem2.hto, subset = 
                         hto.class == 'singlet' &
                         snp.class == 'singlet' &
                         scDblF.RNA.class == 'singlet' &
...
quantile(gem2.singlet$nCount_HTO, 0.999)
quantile(gem2.singlet$nCount_ADT, 0.999)
quantile(gem2.singlet$nCount_RNA, 0.999)
quantile(gem2.singlet$nFeature_RNA, 0.999)
quantile(gem2.singlet$nCount_ATAC, 0.999)
quantile(gem2.singlet$nFeature_ATAC, 0.999)

# Visualize outlier cutoff for UMI counts across modalities
Idents(gem2.singlet) <- 'hto.sort'
VlnPlot(gem2.singlet, features = 'nCount_HTO', pt.size = 0.1, log = TRUE) + NoLegend() + geom_hline(yintercept = 4000)
VlnPlot(gem2.singlet, features = 'nCount_ADT', pt.size = 0.1, log = TRUE) + NoLegend() + geom_hline(yintercept = 4500)
VlnPlot(gem2.singlet, features = 'nCount_RNA', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 20000)
VlnPlot(gem2.singlet, features = 'nFeature_RNA', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 5000)
VlnPlot(gem2.singlet, features = 'nCount_ATAC', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 50000)
VlnPlot(gem2.singlet, features = 'nFeature_ATAC', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 15000)

# Filter outlier UMI droplets from object
gem2.singlet.umi <- subset(gem2.singlet, subset = 
                             nCount_HTO < 4000 &
                             nCount_ADT < 4500 &
                             nCount_RNA < 20000 &
                             nFeature_RNA < 5000 &
...
(ncol(gem2.hto)) # 5215 input
(ncol(gem2.singlet)) # 4119 singlets
(ncol(gem2.singlet) - ncol(gem2.singlet.umi)) # 16 removed
(ncol(gem2.singlet.umi)) # 4103 remaining

# Visualize new UMI distribution across modalities
VlnPlot(gem2.singlet.umi, features = 'nCount_HTO', pt.size = 0.1, log = TRUE) + NoLegend() + geom_hline(yintercept = 4000) + ylim(0,4500)
VlnPlot(gem2.singlet.umi, features = 'nCount_ADT', pt.size = 0.1, log = TRUE) + NoLegend() + geom_hline(yintercept = 4500) + ylim(0,5000)
VlnPlot(gem2.singlet.umi, features = 'nCount_RNA', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 20000) + ylim(0,20500)
VlnPlot(gem2.singlet.umi, features = 'nFeature_RNA', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 5000) + ylim(0,5500)
VlnPlot(gem2.singlet.umi, features = 'nCount_ATAC', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 50000) + ylim(0,50500)
VlnPlot(gem2.singlet.umi, features = 'nFeature_ATAC', pt.size = 0.1, log = FALSE) + NoLegend() + geom_hline(yintercept = 15000) + ylim(0,15500)

# 12) Save singlet Seurat object ----
saveRDS(gem2.singlet.umi, '/filepath/step8_filter_multiplets/gem2.singlet.umi.rds')
