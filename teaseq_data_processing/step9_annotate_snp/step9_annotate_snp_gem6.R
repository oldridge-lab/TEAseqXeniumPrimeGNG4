# Step 9 - Annotate SNP Profile (GEM6) ----
...
gem6.singlet <- readRDS("/filepath/step8_filter_multiplets/gem6.singlet.umi.rds")

# 3) Visualize signal for each unique HTO across SNP profiles ----

# Set idents to SNP-based souporcell profiles
Idents(gem6.singlet) <- "snp.profile"

# CLR-normalize HTO counts across cells
DefaultAssay(gem6.singlet) <- "HTO"
gem6.singlet <- NormalizeData(gem6.singlet, assay= "HTO", normalization.method = "CLR", margin = 2)

# Plot HTO counts for each SNP profile
VlnPlot(gem6.singlet, features = c("HTO-1","HTO-2","HTO-3","HTO-4","HTO-5","HTO-6","HTO-7","HTO-8","HTO-9","HTO-10","HTO-11","HTO-12","HTO-13","HTO-14","HTO-15","HTO-16"), 
...
gem6.singlet$snp.donor <- gem6.singlet$snp.profile
Idents(gem6.singlet) <- "snp.donor"
gem6.annot <- RenameIdents(gem6.singlet,
                           "6" = "P1",
                           "1" = "P2",
                           "3" = "P3",
                           "0" = "P4",
                           ...
                           "4" = "T3",
                           "5" = "T4")
gem6.annot$snp.donor <- Idents(gem6.annot)

# Plot HTO counts for each annotated SNP donor
VlnPlot(gem6.annot, features = c("HTO-1","HTO-2","HTO-3","HTO-4","HTO-5","HTO-6","HTO-7","HTO-8","HTO-9","HTO-10","HTO-11","HTO-12","HTO-13","HTO-14","HTO-15","HTO-16"), 
...
Idents(gem6.annot) <- "snp.donor"
gem6.annot$snp.tissue <- gem6.annot$snp.donor
Idents(gem6.annot) <- "snp.tissue"
gem6.annot <- RenameIdents(gem6.annot, 
                            "P1" = "PBMC",
                            "P2" = "PBMC",
                            "P3" = "PBMC",
                            "P4" = "PBMC",
                            ...
                            "T3" = "Tonsil",
                            "T4" = "Tonsil")
gem6.annot$snp.tissue <- Idents(gem6.annot)
...
hto_df <- t(as.data.frame(GetAssayData(subset(gem6.annot), assay = "HTO")))
snp <- as.data.frame(gem6.annot$snp.donor)
...
saveRDS(gem6.annot, "/filepath/step9_annotate_snp/gem6.annot.snp.rds")
