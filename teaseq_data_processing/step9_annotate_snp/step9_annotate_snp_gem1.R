# Step 9 - Annotate SNP Profile (GEM1) ----
...
gem1.singlet <- readRDS("/filepath/step8_filter_multiplets/gem1.singlet.umi.rds")

# 3) Visualize signal for each unique HTO across SNP profiles ----

# Set idents to SNP-based souporcell profiles
Idents(gem1.singlet) <- "snp.profile"

# CLR-normalize HTO counts across cells
DefaultAssay(gem1.singlet) <- "HTO"
gem1.singlet <- NormalizeData(gem1.singlet, assay= "HTO", normalization.method = "CLR", margin = 2)

# Plot HTO counts for each SNP profile
VlnPlot(gem1.singlet, features = c("HTO-1","HTO-2","HTO-3","HTO-4","HTO-5","HTO-6","HTO-7","HTO-8","HTO-9","HTO-10","HTO-11","HTO-12","HTO-13","HTO-14","HTO-15","HTO-16"), 
...
gem1.singlet$snp.donor <- gem1.singlet$snp.profile
Idents(gem1.singlet) <- "snp.donor"
gem1.annot <- RenameIdents(gem1.singlet,
                           "4" = "P1",
                           "2" = "P2",
                           "0" = "P3",
                           "5" = "P4",
                           ...
                           "3" = "T3",
                           "7" = "T4")
gem1.annot$snp.donor <- Idents(gem1.annot)

# Plot HTO counts for each annotated SNP donor
VlnPlot(gem1.annot, features = c("HTO-1","HTO-2","HTO-3","HTO-4","HTO-5","HTO-6","HTO-7","HTO-8","HTO-9","HTO-10","HTO-11","HTO-12","HTO-13","HTO-14","HTO-15","HTO-16"), 
...
Idents(gem1.annot) <- "snp.donor"
gem1.annot$snp.tissue <- gem1.annot$snp.donor
Idents(gem1.annot) <- "snp.tissue"
gem1.annot <- RenameIdents(gem1.annot, 
                            "P1" = "PBMC",
                            "P2" = "PBMC",
                            "P3" = "PBMC",
                            "P4" = "PBMC",
                            ...
                            "T3" = "Tonsil",
                            "T4" = "Tonsil")
gem1.annot$snp.tissue <- Idents(gem1.annot)
...
hto_df <- t(as.data.frame(GetAssayData(subset(gem1.annot), assay = "HTO")))
snp <- as.data.frame(gem1.annot$snp.donor)
...
saveRDS(gem1.annot, "/filepath/step9_annotate_snp/gem1.annot.snp.rds")
