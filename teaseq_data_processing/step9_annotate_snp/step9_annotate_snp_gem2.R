# Step 9 - Annotate SNP Profile (GEM2) ----
...
gem2.singlet <- readRDS("/filepath/step8_filter_multiplets/gem2.singlet.umi.rds")

# 3) Visualize signal for each unique HTO across SNP profiles ----

# Set idents to SNP-based souporcell profiles
Idents(gem2.singlet) <- "snp.profile"

# CLR-normalize HTO counts across cells
DefaultAssay(gem2.singlet) <- "HTO"
gem2.singlet <- NormalizeData(gem2.singlet, assay= "HTO", normalization.method = "CLR", margin = 2)

# Plot HTO counts for each SNP profile
VlnPlot(gem2.singlet, features = c("HTO-1","HTO-2","HTO-3","HTO-4","HTO-5","HTO-6","HTO-7","HTO-8","HTO-9","HTO-10","HTO-11","HTO-12","HTO-13","HTO-14","HTO-15","HTO-16"), 
...
gem2.singlet$snp.donor <- gem2.singlet$snp.profile
Idents(gem2.singlet) <- "snp.donor"
gem2.annot <- RenameIdents(gem2.singlet,
                           "4" = "P1",
                           "6" = "P2",
                           "0" = "P3",
                           "1" = "P4",
                           ...
                           "3" = "T3",
                           "2" = "T4")
gem2.annot$snp.donor <- Idents(gem2.annot)

# Plot HTO counts for each annotated SNP donor
VlnPlot(gem2.annot, features = c("HTO-1","HTO-2","HTO-3","HTO-4","HTO-5","HTO-6","HTO-7","HTO-8","HTO-9","HTO-10","HTO-11","HTO-12","HTO-13","HTO-14","HTO-15","HTO-16"), 
...
Idents(gem2.annot) <- "snp.donor"
gem2.annot$snp.tissue <- gem2.annot$snp.donor
Idents(gem2.annot) <- "snp.tissue"
gem2.annot <- RenameIdents(gem2.annot, 
                            "P1" = "PBMC",
                            "P2" = "PBMC",
                            "P3" = "PBMC",
                            "P4" = "PBMC",
                            ...
                            "T3" = "Tonsil",
                            "T4" = "Tonsil")
gem2.annot$snp.tissue <- Idents(gem2.annot)
...
hto_df <- t(as.data.frame(GetAssayData(subset(gem2.annot), assay = "HTO")))
snp <- as.data.frame(gem2.annot$snp.donor)
...
saveRDS(gem2.annot, "/filepath/step9_annotate_snp/gem2.annot.snp.rds")
