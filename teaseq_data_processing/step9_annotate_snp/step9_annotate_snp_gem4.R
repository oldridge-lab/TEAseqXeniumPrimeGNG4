# Step 9 - Annotate SNP Profile (GEM4) ----
...
gem4.singlet <- readRDS("/filepath/step8_filter_multiplets/gem4.singlet.umi.rds")

# 3) Visualize signal for each unique HTO across SNP profiles ----

# Set idents to SNP-based souporcell profiles
Idents(gem4.singlet) <- "snp.profile"

# CLR-normalize HTO counts across cells
DefaultAssay(gem4.singlet) <- "HTO"
gem4.singlet <- NormalizeData(gem4.singlet, assay= "HTO", normalization.method = "CLR", margin = 2)

# Plot HTO counts for each SNP profile
VlnPlot(gem4.singlet, features = c("HTO-1","HTO-2","HTO-3","HTO-4","HTO-5","HTO-6","HTO-7","HTO-8","HTO-9","HTO-10","HTO-11","HTO-12","HTO-13","HTO-14","HTO-15","HTO-16"), 
...
gem4.singlet$snp.donor <- gem4.singlet$snp.profile
Idents(gem4.singlet) <- "snp.donor"
gem4.annot <- RenameIdents(gem4.singlet,
                           "4" = "P1",
                           "7" = "P2",
                           "5" = "P3",
                           "0" = "P4",
                           ...
                           "1" = "T3",
                           "2" = "T4")
gem4.annot$snp.donor <- Idents(gem4.annot)

# Plot HTO counts for each annotated SNP donor
VlnPlot(gem4.annot, features = c("HTO-1","HTO-2","HTO-3","HTO-4","HTO-5","HTO-6","HTO-7","HTO-8","HTO-9","HTO-10","HTO-11","HTO-12","HTO-13","HTO-14","HTO-15","HTO-16"), 
...
Idents(gem4.annot) <- "snp.donor"
gem4.annot$snp.tissue <- gem4.annot$snp.donor
Idents(gem4.annot) <- "snp.tissue"
gem4.annot <- RenameIdents(gem4.annot, 
                            "P1" = "PBMC",
                            "P2" = "PBMC",
                            "P3" = "PBMC",
                            "P4" = "PBMC",
                            ...
                            "T3" = "Tonsil",
                            "T4" = "Tonsil")
gem4.annot$snp.tissue <- Idents(gem4.annot)
...
hto_df <- t(as.data.frame(GetAssayData(subset(gem4.annot), assay = "HTO")))
snp <- as.data.frame(gem4.annot$snp.donor)
...
saveRDS(gem4.annot, "/filepath/step9_annotate_snp/gem4.annot.snp.rds")
