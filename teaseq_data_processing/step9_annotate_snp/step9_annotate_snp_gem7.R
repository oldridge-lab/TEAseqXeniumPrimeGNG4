# Step 9 - Annotate SNP Profile (GEM7) ----
...
gem7.singlet <- readRDS("/filepath/step8_filter_multiplets/gem7.singlet.umi.rds")

# 3) Visualize signal for each unique HTO across SNP profiles ----

# Set idents to SNP-based souporcell profiles
Idents(gem7.singlet) <- "snp.profile"

# CLR-normalize HTO counts across cells
DefaultAssay(gem7.singlet) <- "HTO"
gem7.singlet <- NormalizeData(gem7.singlet, assay= "HTO", normalization.method = "CLR", margin = 2)

# Plot HTO counts for each SNP profile
VlnPlot(gem7.singlet, features = c("HTO-1","HTO-2","HTO-3","HTO-4","HTO-5","HTO-6","HTO-7","HTO-8","HTO-9","HTO-10","HTO-11","HTO-12","HTO-13","HTO-14","HTO-15","HTO-16"), 
...
gem7.singlet$snp.donor <- gem7.singlet$snp.profile
Idents(gem7.singlet) <- "snp.donor"
gem7.annot <- RenameIdents(gem7.singlet,
                           "5" = "P1",
                           "2" = "P2",
                           "3" = "P3",
                           "1" = "P4",
                           ...
                           "7" = "T3",
                           "4" = "T4")
gem7.annot$snp.donor <- Idents(gem7.annot)

# Plot HTO counts for each annotated SNP donor
VlnPlot(gem7.annot, features = c("HTO-1","HTO-2","HTO-3","HTO-4","HTO-5","HTO-6","HTO-7","HTO-8","HTO-9","HTO-10","HTO-11","HTO-12","HTO-13","HTO-14","HTO-15","HTO-16"), 
...
Idents(gem7.annot) <- "snp.donor"
gem7.annot$snp.tissue <- gem7.annot$snp.donor
Idents(gem7.annot) <- "snp.tissue"
gem7.annot <- RenameIdents(gem7.annot, 
                            "P1" = "PBMC",
                            "P2" = "PBMC",
                            "P3" = "PBMC",
                            "P4" = "PBMC",
                            ...
                            "T3" = "Tonsil",
                            "T4" = "Tonsil")
gem7.annot$snp.tissue <- Idents(gem7.annot)
...
hto_df <- t(as.data.frame(GetAssayData(subset(gem7.annot), assay = "HTO")))
snp <- as.data.frame(gem7.annot$snp.donor)
...
saveRDS(gem7.annot, "/filepath/step9_annotate_snp/gem7.annot.snp.rds")
