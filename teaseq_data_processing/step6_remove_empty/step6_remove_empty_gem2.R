# Step 6 - Remove Empty Droplets (GEM2)
...
gem2.preproc <- readRDS('/filepath/step5_preprocess_gems/gem2_preprocessed.rds')

# 3) Calculate empty droplet filtering thresholds for RNA, ATAC, ADT, and HTO modalities ----

# RNA - UMI and Feature thresholds
quantile(gem2.preproc$nCount_RNA, 0.02)
rna.umi.thresh <- 350
quantile(gem2.preproc$nFeature_RNA, 0.02)
rna.feat.thresh <- 250

# ATAC - UMI and Feature thresholds
quantile(gem2.preproc$nCount_ATAC, 0.02)
atac.umi.thresh <- 700
quantile(gem2.preproc$nFeature_ATAC, 0.02)
atac.feat.thresh <- 350

# ADT - only UMI threshold
quantile(gem2.preproc$nCount_ADT, 0.02)
adt.umi.thresh <- 175

# HTO - only UMI threshold
quantile(gem2.preproc$nCount_HTO, 0.02)
hto.umi.thresh <- 50

# Mitochondrial UMI % threshold
DefaultAssay(gem2.preproc) <- 'RNA'
gem2.preproc[["percent.mt"]] <- PercentageFeatureSet(gem2.preproc, pattern = "^MT-") 
quantile(gem2.preproc$percent.mt, 0.98)
mito.thresh <- 30

# 4) Visualize filtering thresholds ----

VlnPlot(gem2.preproc, features = 'nCount_RNA', pt.size = 0.1, log = TRUE) + geom_hline(yintercept = rna.umi.thresh)
VlnPlot(gem2.preproc, features = 'nFeature_RNA', pt.size = 0.1, log = TRUE) + geom_hline(yintercept = rna.feat.thresh)
VlnPlot(gem2.preproc, features = 'nCount_ATAC', pt.size = 0.1, log = TRUE) + geom_hline(yintercept = atac.umi.thresh)
VlnPlot(gem2.preproc, features = 'nFeature_ATAC', pt.size = 0.1, log = TRUE) + geom_hline(yintercept = atac.feat.thresh)
VlnPlot(gem2.preproc, features = 'nCount_ADT', pt.size = 0.1, log = TRUE) + geom_hline(yintercept = adt.umi.thresh)
VlnPlot(gem2.preproc, features = 'nCount_HTO', pt.size = 0.1, log = TRUE) + geom_hline(yintercept = hto.umi.thresh)
VlnPlot(gem2.preproc, features = 'percent.mt', pt.size = 0.1, log = TRUE) + geom_hline(yintercept = mito.thresh)

# 5) Filter out empty droplets ----

gem2.filt <- subset(gem2.preproc, subset = 
...
ncol(gem2.preproc) # 5807 droplets initial
ncol(gem2.preproc) - ncol(gem2.filt) # 592 removed
ncol(gem2.filt) # 5215 cell droplets

# 6) Save filtered Seurat object ----
saveRDS(gem2.filt,'/filepath/step6_remove_empty/gem2.filt.rds')
