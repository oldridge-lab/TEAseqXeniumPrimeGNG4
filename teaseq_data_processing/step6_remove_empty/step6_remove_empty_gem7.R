# Step 6 - Remove Empty Droplets (GEM7)
...
gem7.preproc <- readRDS('/filepath/step5_preprocess_gems/gem7_preprocessed.rds')

# 3) Calculate empty droplet filtering thresholds for RNA, ATAC, ADT, and HTO modalities ----

# RNA - UMI and Feature thresholds
quantile(gem7.preproc$nCount_RNA, 0.02)
rna.umi.thresh <- 350
quantile(gem7.preproc$nFeature_RNA, 0.02)
rna.feat.thresh <- 250

# ATAC - UMI and Feature thresholds
quantile(gem7.preproc$nCount_ATAC, 0.02)
atac.umi.thresh <- 700
quantile(gem7.preproc$nFeature_ATAC, 0.02)
atac.feat.thresh <- 350

# ADT - only UMI threshold
quantile(gem7.preproc$nCount_ADT, 0.02)
adt.umi.thresh <- 175

# HTO - only UMI threshold
quantile(gem7.preproc$nCount_HTO, 0.02)
hto.umi.thresh <- 50

# Mitochondrial UMI % threshold
DefaultAssay(gem7.preproc) <- 'RNA'
gem7.preproc[["percent.mt"]] <- PercentageFeatureSet(gem7.preproc, pattern = "^MT-") 
quantile(gem7.preproc$percent.mt, 0.98)
mito.thresh <- 30

# 4) Visualize filtering thresholds ----

VlnPlot(gem7.preproc, features = 'nCount_RNA', pt.size = 0.1, log = TRUE) + geom_hline(yintercept = rna.umi.thresh)
VlnPlot(gem7.preproc, features = 'nFeature_RNA', pt.size = 0.1, log = TRUE) + geom_hline(yintercept = rna.feat.thresh)
VlnPlot(gem7.preproc, features = 'nCount_ATAC', pt.size = 0.1, log = TRUE) + geom_hline(yintercept = atac.umi.thresh)
VlnPlot(gem7.preproc, features = 'nFeature_ATAC', pt.size = 0.1, log = TRUE) + geom_hline(yintercept = atac.feat.thresh)
VlnPlot(gem7.preproc, features = 'nCount_ADT', pt.size = 0.1, log = TRUE) + geom_hline(yintercept = adt.umi.thresh)
VlnPlot(gem7.preproc, features = 'nCount_HTO', pt.size = 0.1, log = TRUE) + geom_hline(yintercept = hto.umi.thresh)
VlnPlot(gem7.preproc, features = 'percent.mt', pt.size = 0.1, log = TRUE) + geom_hline(yintercept = mito.thresh)

# 5) Filter out empty droplets ----

gem7.filt <- subset(gem7.preproc, subset = 
...
ncol(gem7.preproc) # 5747 droplets initial
ncol(gem7.preproc) - ncol(gem7.filt) # 456 empty droplets removed (528 with % MT filter)
ncol(gem7.filt) # 5219 cell droplets

# 6) Save filtered Seurat object ----
saveRDS(gem7.filt,'/filepath/step6_remove_empty/gem7.filt.rds')
