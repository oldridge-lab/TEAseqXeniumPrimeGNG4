# TEAseq Preprocessing Step 5 (GEM1) ----
...
multi_1.data <- Read10X("/filepath/cellranger_multi_output/GEM1_multi/outs/multi/count/raw_feature_bc_matrix", unique.features = TRUE)

# Import cellranger-arc count filtered RNA and ATAC feature barcode matrix
arc_1.data <- Read10X("/filepath/cellranger-arc_output/GEM1_ARC/outs/filtered_feature_bc_matrix", unique.features = TRUE)

# 3) Remove ambient RNA using SoupX----

# Load in cellranger-arc count filtered count matrix in SoupX-readable format
soup_1 <- load10X("/filepath/cellranger-arc_output/GEM1_ARC/outs/", use.names = TRUE, unique.features = TRUE)

# Perform automatic estimation of ambient RNA
soup_1 <- autoEstCont(soup_1)

# Adjust RNA count matrix to remove estimated ambient counts
clean_1 <- adjustCounts(soup_1)

# 4) Create Seurat object for GEM1 using cleaned RNA matrix, to which we will add all other modalities ----
obj_gem1 <- CreateSeuratObject(counts = clean_1, project = "GEM_1")
...
atac_1.counts <- arc_1.data$Peaks
...
multi_1_barcodes <- colnames(multi_1.data[["Gene Expression"]])
arc_1_barcodes <- colnames(arc_1.data[["Gene Expression"]])

# Find barcode intersect between cellranger-arc and cellranger multi output matrices
common_1_barcodes <- intersect(multi_1_barcodes, arc_1_barcodes)

# Subset Seurat object on common barcodes
obj_gem1_subset <- subset(obj_gem1, cells = common_1_barcodes)
obj_gem1_subset$GEM <- "GEM_1"

# 7) Add HTO assay to Seurat object ----

# Create cell x HTO feature matrix using cellranger multi output
hto_1_count <- multi_1.data
hto_1_count[["Antibody Capture"]] <- hto_1_count[["Antibody Capture"]][grep("HTO", rownames(hto_1_count[["Antibody Capture"]])), ]
hto_1_count[["Gene Expression"]] <- NULL
hto_1 <- CreateSeuratObject(counts = hto_1_count)
hto_1_subset <- subset(hto_1, cells = common_1_barcodes)
RenameAssays(hto_1_subset, assay.name = 'RNA', new.assay.name = 'HTO')
obj_gem1_subset[["HTO"]] <- CreateAssayObject(counts = hto_1_subset@assays$RNA$counts)  

# 8) Add ADT assay to Seurat object ----

# Create cell x ADT feature matrix using cellranger multi output
adt_1_count <- multi_1.data
adt_1_count[["Antibody Capture"]] <- adt_1_count[["Antibody Capture"]][-grep("HTO", rownames(adt_1_count[["Antibody Capture"]])), ]
adt_1_count[["Gene Expression"]] <- NULL
adt_1 <- CreateSeuratObject(counts = adt_1_count)
adt_1_subset <- subset(adt_1, cells = common_1_barcodes)
RenameAssays(adt_1_subset, assay.name = 'RNA', new.assay.name = 'ADT')
obj_gem1_subset[["ADT"]] <- CreateAssayObject(counts = adt_1_subset@assays$RNA$counts)  

# 9) Add SNP-based demultiplexing metadata from Souporcell to Seurat object ----

# Import souporcell output SNP-based sample demultiplexing cluster file
snp.metadata.gem1 <- as.data.frame(read_tsv("/filepath/step4_snp_demultiplex/gem1/clusters.tsv"))

# Add souporcell output metadata to Seurat object
obj_gem1_subset <- AddMetaData(obj_gem1_subset, metadata = snp.metadata.gem1)

# 10) Save pre-processed GEM object ----
saveRDS(obj_gem1_subset, "/filepath/step5_preprocess_gems/gem1_preprocessed.rds")
