# TEAseq Preprocessing Step 5 (GEM3) ----
...
multi_3.data <- Read10X("/filepath/cellranger_multi_output_newlib/GEM3_multi/outs/multi/count/raw_feature_bc_matrix", unique.features = TRUE)

# Import cellranger-arc count filtered RNA and ATAC feature barcode matrix
arc_3.data <- Read10X("/filepath/cellranger-arc_output/GEM3_ARC/outs/filtered_feature_bc_matrix", unique.features = TRUE)

# 3) Remove ambient RNA using SoupX----

# Load in cellranger-arc count filtered count matrix in SoupX-readable format
soup_3 <- load10X("/filepath/cellranger-arc_output/GEM3_ARC/outs/", use.names = TRUE, unique.features = TRUE)

# Perform automatic estimatation of ambient RNA
soup_3 <- autoEstCont(soup_3)

# Adjust RNA count matrix to remove estimated ambient counts
clean_3 <- adjustCounts(soup_3)

# 4) Create Seurat object for GEM3 using cleaned RNA matrix, to which we will add all other modalities ----
obj_gem3 <- CreateSeuratObject(counts = clean_3, project = "GEM_3")
...
atac_3.counts <- arc_3.data$Peaks
...
chrom_assay_3 <- CreateChromatinAssay(
  counts = atac_3.counts,
  sep = c(":", "-"),
...
multi_3_barcodes <- colnames(multi_3.data[["Gene Expression"]])
arc_3_barcodes <- colnames(arc_3.data[["Gene Expression"]])

# Find barcode intersect between cellranger-arc and cellranger multi output matrices
common_3_barcodes <- intersect(multi_3_barcodes, arc_3_barcodes)

# Subset Seurat object on common barcodes
obj_gem3_subset <- subset(obj_gem3, cells = common_3_barcodes)
obj_gem3_subset$GEM <- "GEM_3"

# 7) Add HTO assay to Seurat object ----

# Create cell x HTO feature matrix using cellranger multi output
hto_3_count <- multi_3.data
hto_3_count[["Antibody Capture"]] <- hto_3_count[["Antibody Capture"]][grep("HTO", rownames(hto_3_count[["Antibody Capture"]])), ]
hto_3_count[["Gene Expression"]] <- NULL
hto_3 <- CreateSeuratObject(counts = hto_3_count)
hto_3_subset <- subset(hto_3, cells = common_3_barcodes)
RenameAssays(hto_3_subset, assay.name = 'RNA', new.assay.name = 'HTO')
obj_gem3_subset[["HTO"]] <- CreateAssayObject(counts = hto_3_subset@assays$RNA$counts)  

# 8) Add ADT assay to Seurat object ----

# Create cell x ADT feature matrix using cellranger multi output
adt_3_count <- multi_3.data
adt_3_count[["Antibody Capture"]] <- adt_3_count[["Antibody Capture"]][-grep("HTO", rownames(adt_3_count[["Antibody Capture"]])), ]
adt_3_count[["Gene Expression"]] <- NULL
adt_3 <- CreateSeuratObject(counts = adt_3_count)
adt_3_subset <- subset(adt_3, cells = common_3_barcodes)
RenameAssays(adt_3_subset, assay.name = 'RNA', new.assay.name = 'ADT')
obj_gem3_subset[["ADT"]] <- CreateAssayObject(counts = adt_3_subset@assays$RNA$counts)  

# 9) Add SNP-based demultiplexing metadata from Souporcell to Seurat object ----

# Import souporcell output SNP-based sample demultiplexing cluster file
snp.metadata.gem3 <- as.data.frame(read_tsv("/filepath/step4_snp_demultiplex/gem3/clusters.tsv"))

# Add souporcell output metadata to Seurat object
obj_gem3_subset <- AddMetaData(obj_gem3_subset, metadata = snp.metadata.gem3)

# 10) Save pre-processed GEM object ----
saveRDS(obj_gem3_subset, "/filepath/step5_preprocess_gems/gem3_preprocessed.rds")
