# TEAseq Preprocessing Step 5 (GEM6) ----
...
multi_6.data <- Read10X("/filepath/cellranger_multi_output_newlib/GEM6_multi/outs/multi/count/raw_feature_bc_matrix", unique.features = TRUE)

# Import cellranger-arc count filtered RNA and ATAC feature barcode matrix
arc_6.data <- Read10X("/filepath/cellranger-arc_output/GEM6_ARC/outs/filtered_feature_bc_matrix", unique.features = TRUE)

# 3) Remove ambient RNA using SoupX----

# Load in cellranger-arc count filtered count matrix in SoupX-readable format
soup_6 <- load10X("/filepath/cellranger-arc_output/GEM6_ARC/outs/", use.names = TRUE, unique.features = TRUE)

# Perform automatic estimation of ambient RNA
soup_6 <- autoEstCont(soup_6)

# Adjust RNA count matrix to remove estimated ambient counts
clean_6 <- adjustCounts(soup_6)

# 4) Create Seurat object for GEM6 using cleaned RNA matrix, to which we will add all other modalities ----
obj_gem6 <- CreateSeuratObject(counts = clean_6, project = "GEM_6")
...
atac_6.counts <- arc_6.data$Peaks
...
chrom_assay_6 <- CreateChromatinAssay(
  counts = atac_6.counts,
  sep = c(":", "-"),
...
multi_6_barcodes <- colnames(multi_6.data[["Gene Expression"]])
arc_6_barcodes <- colnames(arc_6.data[["Gene Expression"]])

# Find barcode intersect between cellranger-arc and cellranger multi output matrices
common_6_barcodes <- intersect(multi_6_barcodes, arc_6_barcodes)

# Subset Seurat object on common barcodes
obj_gem6_subset <- subset(obj_gem6, cells = common_6_barcodes)
obj_gem6_subset$GEM <- "GEM_6"

# 7) Add HTO assay to Seurat object ----

# Create cell x HTO feature matrix using cellranger multi output
hto_6_count <- multi_6.data
hto_6_count[["Antibody Capture"]] <- hto_6_count[["Antibody Capture"]][grep("HTO", rownames(hto_6_count[["Antibody Capture"]])), ]
hto_6_count[["Gene Expression"]] <- NULL
hto_6 <- CreateSeuratObject(counts = hto_6_count)
hto_6_subset <- subset(hto_6, cells = common_6_barcodes)
RenameAssays(hto_6_subset, assay.name = 'RNA', new.assay.name = 'HTO')
obj_gem6_subset[["HTO"]] <- CreateAssayObject(counts = hto_6_subset@assays$RNA$counts)  

# 8) Add ADT assay to Seurat object ----

# Create cell x ADT feature matrix using cellranger multi output
adt_6_count <- multi_6.data
adt_6_count[["Antibody Capture"]] <- adt_6_count[["Antibody Capture"]][-grep("HTO", rownames(adt_6_count[["Antibody Capture"]])), ]
adt_6_count[["Gene Expression"]] <- NULL
adt_6 <- CreateSeuratObject(counts = adt_6_count)
adt_6_subset <- subset(adt_6, cells = common_6_barcodes)
RenameAssays(adt_6_subset, assay.name = 'RNA', new.assay.name = 'ADT')
obj_gem6_subset[["ADT"]] <- CreateAssayObject(counts = adt_6_subset@assays$RNA$counts)  

# 9) Add SNP-based demultiplexing metadata from Souporcell to Seurat object ----

# Import souporcell output SNP-based sample demultiplexing cluster file
snp.metadata.gem6 <- as.data.frame(read_tsv("/filepath/step4_snp_demultiplex/gem6/clusters.tsv"))

# Add souporcell output metadata to Seurat object
obj_gem6_subset <- AddMetaData(obj_gem6_subset, metadata = snp.metadata.gem6)

# 10) Save pre-processed GEM object ----
saveRDS(obj_gem6_subset, "/filepath/step5_preprocess_gems/gem6_preprocessed.rds")
