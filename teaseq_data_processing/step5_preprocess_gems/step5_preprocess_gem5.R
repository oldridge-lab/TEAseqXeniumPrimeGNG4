# TEAseq Preprocessing Step 5 (GEM5) ----
...
multi_5.data <- Read10X("/filepath/cellranger_multi_output_newlib/GEM5_multi/outs/multi/count/raw_feature_bc_matrix", unique.features = TRUE)

# Import cellranger-arc count filtered RNA and ATAC feature barcode matrix
arc_5.data <- Read10X("/filepath/cellranger-arc_output/GEM5_ARC/outs/filtered_feature_bc_matrix", unique.features = TRUE)

# 3) Remove ambient RNA using SoupX----

# Load in cellranger-arc count filtered count matrix in SoupX-readable format
soup_5 <- load10X("/filepath/cellranger-arc_output/GEM5_ARC/outs/", use.names = TRUE, unique.features = TRUE)

# Perform automatic estimation of ambient RNA
soup_5 <- autoEstCont(soup_5)

# Adjust RNA count matrix to remove estimated ambient counts
clean_5 <- adjustCounts(soup_5)

# 4) Create Seurat object for GEM5 using cleaned RNA matrix, to which we will add all other modalities ----
obj_gem5 <- CreateSeuratObject(counts = clean_5, project = "GEM_5")
...
atac_5.counts <- arc_5.data$Peaks
...
chrom_assay_5 <- CreateChromatinAssay(
  counts = atac_5.counts,
  sep = c(":", "-"),
...
multi_5_barcodes <- colnames(multi_5.data[["Gene Expression"]])
arc_5_barcodes <- colnames(arc_5.data[["Gene Expression"]]) 
# Find barcode intersect between cellranger-arc and cellranger multi output matrices

common_5_barcodes <- intersect(multi_5_barcodes, arc_5_barcodes)

# Subset Seurat object on common barcodes
obj_gem5_subset <- subset(obj_gem5, cells = common_5_barcodes)
obj_gem5_subset$GEM <- "GEM_5"

# 7) Add HTO assay to Seurat object ----

# Create cell x HTO feature matrix using cellranger multi output
hto_5_count <- multi_5.data
hto_5_count[["Antibody Capture"]] <- hto_5_count[["Antibody Capture"]][grep("HTO", rownames(hto_5_count[["Antibody Capture"]])), ]
hto_5_count[["Gene Expression"]] <- NULL
hto_5 <- CreateSeuratObject(counts = hto_5_count)
hto_5_subset <- subset(hto_5, cells = common_5_barcodes)
RenameAssays(hto_5_subset, assay.name = 'RNA', new.assay.name = 'HTO')
obj_gem5_subset[["HTO"]] <- CreateAssayObject(counts = hto_5_subset@assays$RNA$counts)  

# 8) Add ADT assay to Seurat object ----

# Create cell x ADT feature matrix using cellranger multi output
adt_5_count <- multi_5.data
adt_5_count[["Antibody Capture"]] <- adt_5_count[["Antibody Capture"]][-grep("HTO", rownames(adt_5_count[["Antibody Capture"]])), ]
adt_5_count[["Gene Expression"]] <- NULL
adt_5 <- CreateSeuratObject(counts = adt_5_count)
adt_5_subset <- subset(adt_5, cells = common_5_barcodes)
RenameAssays(adt_5_subset, assay.name = 'RNA', new.assay.name = 'ADT')
obj_gem5_subset[["ADT"]] <- CreateAssayObject(counts = adt_5_subset@assays$RNA$counts)  

# 9) Add SNP-based demultiplexing metadata from Souporcell to Seurat object ----

# Import souporcell output SNP-based sample demultiplexing cluster file
snp.metadata.gem5 <- as.data.frame(read_tsv("/filepath/step4_snp_demultiplex/gem5/clusters.tsv"))

# Add souporcell output metadata to Seurat object
obj_gem5_subset <- AddMetaData(obj_gem5_subset, metadata = snp.metadata.gem5)

# 10) Save pre-processed GEM object ----
saveRDS(obj_gem5_subset, "/filepath/step5_preprocess_gems/gem5_preprocessed.rds")
