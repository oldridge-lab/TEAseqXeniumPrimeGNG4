# TEAseq Preprocessing Step 5 (GEM4) ----
...
multi_4.data <- Read10X("/filepath/GEM4_multi/outs/multi/count/raw_feature_bc_matrix", unique.features = TRUE)

# Import cellranger-arc count filtered RNA and ATAC feature barcode matrix
arc_4.data <- Read10X("/filepath/cellranger-arc_output/GEM4_ARC/outs/filtered_feature_bc_matrix", unique.features = TRUE)

# 3) Remove ambient RNA using SoupX----

# Load in cellranger-arc count filtered count matrix in SoupX-readable format
soup_4 <- load10X("/filepath/cellranger-arc_output/GEM4_ARC/outs/", use.names = TRUE, unique.features = TRUE)

# Perform automatic estimation of ambient RNA
soup_4 <- autoEstCont(soup_4)

# Adjust RNA count matrix to remove estimated ambient counts
clean_4 <- adjustCounts(soup_4)

# 4) Create Seurat object for GEM4 using cleaned RNA matrix, to which we will add all other modalities ----
obj_gem4 <- CreateSeuratObject(counts = clean_4, project = "GEM_4")
...
atac_4.counts <- arc_4.data$Peaks
...
chrom_assay_4 <- CreateChromatinAssay(
  counts = atac_4.counts,
  sep = c(":", "-"),
...
multi_4_barcodes <- colnames(multi_4.data[["Gene Expression"]]) 
arc_4_barcodes <- colnames(arc_4.data[["Gene Expression"]]) 

# Find barcode intersect between cellranger-arc and cellranger multi output matrices
common_4_barcodes <- intersect(multi_4_barcodes, arc_4_barcodes) 

# Subset Seurat object on common barcodes
obj_gem4_subset <- subset(obj_gem4, cells = common_4_barcodes)
obj_gem4_subset$GEM <- "GEM_4"

# 7) Add HTO assay to Seurat object ----

# Create cell x HTO feature matrix using cellranger multi output
hto_4_count <- multi_4.data
hto_4_count[["Antibody Capture"]] <- hto_4_count[["Antibody Capture"]][grep("HTO", rownames(hto_4_count[["Antibody Capture"]])), ]
hto_4_count[["Gene Expression"]] <- NULL
hto_4 <- CreateSeuratObject(counts = hto_4_count)
hto_4_subset <- subset(hto_4, cells = common_4_barcodes)
RenameAssays(hto_4_subset, assay.name = 'RNA', new.assay.name = 'HTO')
obj_gem4_subset[["HTO"]] <- CreateAssayObject(counts = hto_4_subset@assays$RNA$counts)  

# 8) Add ADT assay to Seurat object ----

# Create cell x ADT feature matrix using cellranger multi output
adt_4_count <- multi_4.data
adt_4_count[["Antibody Capture"]] <- adt_4_count[["Antibody Capture"]][-grep("HTO", rownames(adt_4_count[["Antibody Capture"]])), ]
adt_4_count[["Gene Expression"]] <- NULL
adt_4 <- CreateSeuratObject(counts = adt_4_count)
adt_4_subset <- subset(adt_4, cells = common_4_barcodes)
RenameAssays(adt_4_subset, assay.name = 'RNA', new.assay.name = 'ADT')
obj_gem4_subset[["ADT"]] <- CreateAssayObject(counts = adt_4_subset@assays$RNA$counts)  

# 9) Add SNP-based demultiplexing metadata from Souporcell to Seurat object ----

# Import souporcell output SNP-based sample demultiplexing cluster file
snp.metadata.gem4 <- as.data.frame(read_tsv("/filepath/step4_snp_demultiplex/gem4/clusters.tsv"))

# Add souporcell output metadata to Seurat object
obj_gem4_subset <- AddMetaData(obj_gem4_subset, metadata = snp.metadata.gem4)

# 10) Save pre-processed GEM object ----
saveRDS(obj_gem4_subset, "/filepath/step5_preprocess_gems/gem4_preprocessed.rds")
