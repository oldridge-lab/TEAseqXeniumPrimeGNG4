...
gem2.filt <- readRDS('/filepath/step6_remove_empty/gem2.filt.rds')

# 3) Calculate % HTO UMI metrics ----

# Calculate % HTO UMI metrics
hto.counts.mtx <- gem2.filt[['HTO']]$counts
...
gem2.filt$pct.hto.max <- hto.counts.mtx.sort.pct[,1]
gem2.filt$pct.hto.2nd <- hto.counts.mtx.sort.pct[,2]
gem2.filt$pct.hto.bg <- rowSums(hto.counts.mtx.sort.pct[,2:16])
...
gem2.filt$hto.max <- hto.max

# 4) Annotate cells based on % HTO UMI metrics ----

# Assign flow sorting group labels from % Max HTO metrics
gem2.filt$hto.sort <- gem2.filt$hto.max
Idents(gem2.filt) <- 'hto.sort'
gem2.filt <- RenameIdents(gem2.filt,
              "HTO-1" = "P1-Bulk", 
              "HTO-2" = "P2-Bulk", 
              "HTO-3" = "P3-Bulk", 
              "HTO-4" = "P4-Bulk", 
              ...
              "HTO-15" = "T3-CD4", 
              "HTO-16" = "T4-CD4")
gem2.filt$hto.sort <- Idents(gem2.filt)
as.data.frame(gem2.filt$hto.sort) %>% 
  ggplot(aes(x = gem2.filt$hto.sort)) + 
  geom_bar(fill = 'azure4') + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Cells per flow sorting group (GEM2)',
       x = 'Flow sorting group',
       y = 'Number of cells') +
  theme_minimal()

# Assign donor labels from % Max HTO metrics
gem2.filt$hto.donor <- gem2.filt$hto.max
Idents(gem2.filt) <- 'hto.donor'
gem2.filt <- RenameIdents(gem2.filt,
                          "HTO-1" = "P1", 
                          "HTO-2" = "P2", 
                          "HTO-3" = "P3", 
                          ...
                          "HTO-15" = "T3", 
                          "HTO-16" = "T4")
gem2.filt$hto.donor <- Idents(gem2.filt)
as.data.frame(gem2.filt$hto.donor) %>% 
  ggplot(aes(x = gem2.filt$hto.donor)) + 
  geom_bar(fill = 'azure4') + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Cells per donor (GEM2)',
       x = 'Donor',
       y = 'Number of cells') +
  theme_minimal()

# Assign tissue type labels from % Max HTO metrics
gem2.filt$hto.tissue <- gem2.filt$hto.max
Idents(gem2.filt) <- 'hto.tissue'
gem2.filt <- RenameIdents(gem2.filt,
                          "HTO-1" = "PBMC", 
                          "HTO-2" = "PBMC", 
                          "HTO-3" = "PBMC", 
                          ...
                          "HTO-15" = "Tonsil", 
                          "HTO-16" = "Tonsil")
gem2.filt$hto.tissue <- Idents(gem2.filt)
as.data.frame(gem2.filt$hto.tissue) %>% 
  ggplot(aes(x = gem2.filt$hto.tissue)) + 
  geom_bar(fill = 'azure4') + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Cells per tissue (GEM2)',
       x = 'Tissue',
       y = 'Number of cells') +
  theme_minimal()

# Clean up excess or redundant metadata columns
gem2.filt$HTO_classification.global <- NULL
gem2.filt$HTO_classification <- NULL
gem2.filt$HTO_margin <- NULL
gem2.filt$HTO_maxID <- NULL
gem2.filt$hash.ID <- NULL
gem2.filt$HTO_secondID <- NULL
gem2.filt$orig.ident <- NULL

# 5) Save HTO-demultiplexed Seurat object ----
saveRDS(gem2.filt, '/filepath/step7_hto_demultiplex/gem2.hto.rds')
