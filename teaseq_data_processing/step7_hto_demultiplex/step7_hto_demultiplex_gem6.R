...
gem6.filt <- readRDS('/filepath/step6_remove_empty/gem6.filt.rds')

# 3) Calculate % HTO UMI metrics ----

# Calculate % HTO UMI metrics
hto.counts.mtx <- gem6.filt[['HTO']]$counts
...
gem6.filt$pct.hto.max <- hto.counts.mtx.sort.pct[,1]
gem6.filt$pct.hto.2nd <- hto.counts.mtx.sort.pct[,2]
gem6.filt$pct.hto.bg <- rowSums(hto.counts.mtx.sort.pct[,2:16])
...
gem6.filt$hto.max <- hto.max

# 4) Annotate cells based on % HTO UMI metrics ----

# Assign flow sorting group labels from % Max HTO metrics
gem6.filt$hto.sort <- gem6.filt$hto.max
Idents(gem6.filt) <- 'hto.sort'
gem6.filt <- RenameIdents(gem6.filt,
              "HTO-1" = "P1-Bulk", 
              "HTO-2" = "P2-Bulk", 
              "HTO-3" = "P3-Bulk", 
              "HTO-4" = "P4-Bulk", 
              ...
              "HTO-15" = "T3-CD4", 
              "HTO-16" = "T4-CD4")
gem6.filt$hto.sort <- Idents(gem6.filt)
as.data.frame(gem6.filt$hto.sort) %>% 
  ggplot(aes(x = gem6.filt$hto.sort)) + 
  geom_bar(fill = 'azure4') + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Cells per flow sorting group (GEM6)',
       x = 'Flow sorting group',
       y = 'Number of cells') +
  theme_minimal()

# Assign donor labels from % Max HTO metrics
gem6.filt$hto.donor <- gem6.filt$hto.max
Idents(gem6.filt) <- 'hto.donor'
gem6.filt <- RenameIdents(gem6.filt,
                          "HTO-1" = "P1", 
                          "HTO-2" = "P2", 
                          "HTO-3" = "P3", 
                          ...
                          "HTO-15" = "T3", 
                          "HTO-16" = "T4")
gem6.filt$hto.donor <- Idents(gem6.filt)
as.data.frame(gem6.filt$hto.donor) %>% 
  ggplot(aes(x = gem6.filt$hto.donor)) + 
  geom_bar(fill = 'azure4') + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Cells per donor (GEM6)',
       x = 'Donor',
       y = 'Number of cells') +
  theme_minimal()

# Assign tissue type labels from % Max HTO metrics
gem6.filt$hto.tissue <- gem6.filt$hto.max
Idents(gem6.filt) <- 'hto.tissue'
gem6.filt <- RenameIdents(gem6.filt,
                          "HTO-1" = "PBMC", 
                          "HTO-2" = "PBMC", 
                          "HTO-3" = "PBMC", 
                          ...
                          "HTO-15" = "Tonsil", 
                          "HTO-16" = "Tonsil")
gem6.filt$hto.tissue <- Idents(gem6.filt)
as.data.frame(gem6.filt$hto.tissue) %>% 
  ggplot(aes(x = gem6.filt$hto.tissue)) + 
  geom_bar(fill = 'azure4') + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Cells per tissue (GEM6)',
       x = 'Tissue',
       y = 'Number of cells') +
  theme_minimal()

# Clean up excess or redundant metadata columns
gem6.filt$HTO_classification.global <- NULL
gem6.filt$HTO_classification <- NULL
gem6.filt$HTO_margin <- NULL
gem6.filt$HTO_maxID <- NULL
gem6.filt$hash.ID <- NULL
gem6.filt$HTO_secondID <- NULL
gem6.filt$orig.ident <- NULL

# 5) Save HTO-demultiplexed Seurat object ----
saveRDS(gem6.filt, '/filepath/step7_hto_demultiplex/gem6.hto.rds')
