...
gem8.filt <- readRDS('/filepath/step6_remove_empty/gem8.filt.rds')

# 3) Calculate % HTO UMI metrics ----

# Calculate % HTO UMI metrics
hto.counts.mtx <- gem8.filt[['HTO']]$counts
...
gem8.filt$pct.hto.max <- hto.counts.mtx.sort.pct[,1]
gem8.filt$pct.hto.2nd <- hto.counts.mtx.sort.pct[,2]
gem8.filt$pct.hto.bg <- rowSums(hto.counts.mtx.sort.pct[,2:16])
...
gem8.filt$hto.max <- hto.max

# 4) Annotate cells based on % HTO UMI metrics ----

# Assign flow sorting group labels from % Max HTO metrics
gem8.filt$hto.sort <- gem8.filt$hto.max
Idents(gem8.filt) <- 'hto.sort'
gem8.filt <- RenameIdents(gem8.filt,
              "HTO-1" = "P1-Bulk", 
              "HTO-2" = "P2-Bulk", 
              "HTO-3" = "P3-Bulk", 
              "HTO-4" = "P4-Bulk", 
              ...
              "HTO-15" = "T3-CD4", 
              "HTO-16" = "T4-CD4")
gem8.filt$hto.sort <- Idents(gem8.filt)
as.data.frame(gem8.filt$hto.sort) %>% 
  ggplot(aes(x = gem8.filt$hto.sort)) + 
  geom_bar(fill = 'azure4') + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Cells per flow sorting group (GEM8)',
       x = 'Flow sorting group',
       y = 'Number of cells') +
  theme_minimal()

# Assign donor labels from % Max HTO metrics
gem8.filt$hto.donor <- gem8.filt$hto.max
Idents(gem8.filt) <- 'hto.donor'
gem8.filt <- RenameIdents(gem8.filt,
                          "HTO-1" = "P1", 
                          "HTO-2" = "P2", 
                          "HTO-3" = "P3", 
                          ...
                          "HTO-15" = "T3", 
                          "HTO-16" = "T4")
gem8.filt$hto.donor <- Idents(gem8.filt)
as.data.frame(gem8.filt$hto.donor) %>% 
  ggplot(aes(x = gem8.filt$hto.donor)) + 
  geom_bar(fill = 'azure4') + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Cells per donor (GEM8)',
       x = 'Donor',
       y = 'Number of cells') +
  theme_minimal()

# Assign tissue type labels from % Max HTO metrics
gem8.filt$hto.tissue <- gem8.filt$hto.max
Idents(gem8.filt) <- 'hto.tissue'
gem8.filt <- RenameIdents(gem8.filt,
                          "HTO-1" = "PBMC", 
                          "HTO-2" = "PBMC", 
                          "HTO-3" = "PBMC", 
                          ...
                          "HTO-15" = "Tonsil", 
                          "HTO-16" = "Tonsil")
gem8.filt$hto.tissue <- Idents(gem8.filt)
as.data.frame(gem8.filt$hto.tissue) %>% 
  ggplot(aes(x = gem8.filt$hto.tissue)) + 
  geom_bar(fill = 'azure4') + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Cells per tissue (GEM8)',
       x = 'Tissue',
       y = 'Number of cells') +
  theme_minimal()

# Clean up excess or redundant metadata columns
gem8.filt$HTO_classification.global <- NULL
gem8.filt$HTO_classification <- NULL
gem8.filt$HTO_margin <- NULL
gem8.filt$HTO_maxID <- NULL
gem8.filt$hash.ID <- NULL
gem8.filt$HTO_secondID <- NULL
gem8.filt$orig.ident <- NULL

# 5) Save HTO-demultiplexed Seurat object ----
saveRDS(gem8.filt, '/filepath/step7_hto_demultiplex/gem8.hto.rds')
