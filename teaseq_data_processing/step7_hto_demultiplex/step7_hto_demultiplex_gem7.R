...
gem7.filt <- readRDS('/filepath/step6_remove_empty/gem7.filt.rds')

# 3) Calculate % HTO UMI metrics ----

# Calculate % HTO UMI metrics
hto.counts.mtx <- gem7.filt[['HTO']]$counts
...
gem7.filt$pct.hto.max <- hto.counts.mtx.sort.pct[,1]
gem7.filt$pct.hto.2nd <- hto.counts.mtx.sort.pct[,2]
gem7.filt$pct.hto.bg <- rowSums(hto.counts.mtx.sort.pct[,2:16])
...
gem7.filt$hto.max <- hto.max

# 4) Annotate cells based on % HTO UMI metrics ----

# Assign flow sorting group labels from % Max HTO metrics
gem7.filt$hto.sort <- gem7.filt$hto.max
Idents(gem7.filt) <- 'hto.sort'
gem7.filt <- RenameIdents(gem7.filt,
              "HTO-1" = "P1-Bulk", 
              "HTO-2" = "P2-Bulk", 
              "HTO-3" = "P3-Bulk", 
              "HTO-4" = "P4-Bulk", 
              ...
              "HTO-15" = "T3-CD4", 
              "HTO-16" = "T4-CD4")
gem7.filt$hto.sort <- Idents(gem7.filt)
as.data.frame(gem7.filt$hto.sort) %>% 
  ggplot(aes(x = gem7.filt$hto.sort)) + 
  geom_bar(fill = 'azure4') + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Cells per flow sorting group (GEM7)',
       x = 'Flow sorting group',
       y = 'Number of cells') +
  theme_minimal()

# Assign donor labels from % Max HTO metrics
gem7.filt$hto.donor <- gem7.filt$hto.max
Idents(gem7.filt) <- 'hto.donor'
gem7.filt <- RenameIdents(gem7.filt,
                          "HTO-1" = "P1", 
                          "HTO-2" = "P2", 
                          "HTO-3" = "P3", 
                          ...
                          "HTO-15" = "T3", 
                          "HTO-16" = "T4")
gem7.filt$hto.donor <- Idents(gem7.filt)
as.data.frame(gem7.filt$hto.donor) %>% 
  ggplot(aes(x = gem7.filt$hto.donor)) + 
  geom_bar(fill = 'azure4') + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Cells per donor (GEM7)',
       x = 'Donor',
       y = 'Number of cells') +
  theme_minimal()

# Assign tissue type labels from % Max HTO metrics
gem7.filt$hto.tissue <- gem7.filt$hto.max
Idents(gem7.filt) <- 'hto.tissue'
gem7.filt <- RenameIdents(gem7.filt,
                          "HTO-1" = "PBMC", 
                          "HTO-2" = "PBMC", 
                          "HTO-3" = "PBMC", 
                          ...
                          "HTO-15" = "Tonsil", 
                          "HTO-16" = "Tonsil")
gem7.filt$hto.tissue <- Idents(gem7.filt)
as.data.frame(gem7.filt$hto.tissue) %>% 
  ggplot(aes(x = gem7.filt$hto.tissue)) + 
  geom_bar(fill = 'azure4') + 
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = -0.5) + 
  labs(title = 'Cells per tissue (GEM7)',
       x = 'Tissue',
       y = 'Number of cells') +
  theme_minimal()

# Clean up excess or redundant metadata columns
gem7.filt$HTO_classification.global <- NULL
gem7.filt$HTO_classification <- NULL
gem7.filt$HTO_margin <- NULL
gem7.filt$HTO_maxID <- NULL
gem7.filt$hash.ID <- NULL
gem7.filt$HTO_secondID <- NULL
gem7.filt$orig.ident <- NULL

# 5) Save HTO-demultiplexed Seurat object ----
saveRDS(gem7.filt, '/filepath/step7_hto_demultiplex/gem7.hto.rds')
